...
echo "Applying permanent USB autosuspend and Audio hotplug fixes with boot ordering..."
...
# This sets the ordering BEFORE display-manager/plasmalogin so that powertop tunes
# the devices before the GUI starts, avoiding the race condition.
sudo mkdir -p /etc/systemd/system/powertop-autotune.service.d
cat << 'EOF' > /tmp/override.conf
[Unit]
# Reset After= dependency and set to run early, before the display manager starts
After=
